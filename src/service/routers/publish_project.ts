...
import { ResponseObject, ResponseObjectDecoder } from '../../common/objs/responseobject';
...
import {ROUTER_PATHS, PublishProjectResponseParam, PublishProjecRequestParam} from '../../common/routers';
import { getFriendPeopleIds, makeCommonResponse } from '../util';

export async function publishProjectRouter(
    ...
    if (!object || object.obj_type() !== AppObjectType.PROJECT) {
    ...
    const projectObject: PublishProjecRequestParam = dr.unwrap();
    ...
    const nonObj = new cyfs.NONObjectInfo(
        projectObject.desc().object_id(),
        projectObject.encode_to_buf().unwrap()
    );
